...
function Sidebar({
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex w-[264px] flex-col overflow-hidden border-r border-sidebar-border bg-sidebar px-5 py-5 shadow-elevated transition-transform lg:translate-x-0 ${
          open ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="flex items-start justify-between gap-3">
          <div className="w-full overflow-hidden rounded-sm border border-brand/70 bg-sidebar p-1.5">
            <Brand />
          </div>
        ...
        <div className="mt-auto hidden lg:block">
          <div className="mb-5 flex justify-center text-brand/80" aria-hidden="true">
            <Leaf className="h-16 w-16 stroke-[1]" />
          </div>
...
function EmptyOrNotice({ title, text }: { title: string; text: string }) {
  return (
    <div className="border border-border bg-card shadow-soft rounded-lg border-dashed px-6 py-10 text-center">
...
function HomePage({
  ...
  return (
    <>
      <section className="relative min-h-[112px] overflow-hidden border-b border-border/55 pb-5 pt-2">
        <img
          src={floralImage}
          width={1920}
          height={1024}
          alt="Arranjo floral elegante"
          className="absolute inset-y-0 right-0 h-full w-[58%] object-cover object-center opacity-80" style={{ filter: "saturate(.7) contrast(1.03) brightness(.62)" }}
        />
        <div className="absolute inset-0" style={{ background: "linear-gradient(90deg, #161512 0%, #161512 48%, rgba(22,21,18,.72) 70%, rgba(22,21,18,.16) 100%)" }} />
      ...
      <section aria-label="Resumo" className="mt-5 grid grid-cols-2 gap-3 xl:grid-cols-4">
        {metrics.map(({ label, value, icon: Icon, trend, trendTone }) => (
          <article key={label} className="flex min-w-0 items-center gap-3 rounded-xl border p-3.5 shadow-soft sm:gap-4 sm:p-4" style={{ background: "linear-gradient(135deg,#f7efe2,#ecddc8)", borderColor: "rgba(213,185,121,.42)", color: "#2b241d" }}>
            <div className="grid h-14 w-14 shrink-0 place-items-center rounded-full" style={{ background: "rgba(212,174,128,.18)", color: "#9a6647" }}>
            ...
            <div className="min-w-0">
              ...
              <p className={`mt-1 text-[11px] ${trendTone === "success" ? "text-emerald-700" : "text-red-700"}`}>
      ...
      <section className="mt-4 grid gap-4 xl:grid-cols-3">
        <article className="border border-border bg-card shadow-soft overflow-hidden rounded-xl">
          ...
          <div className="px-4 py-1">
            {shownEvents.length ? (
              shownEvents.map((event, index) => (
                <button
                  key={event.id}
                  className="flex w-full items-center gap-3 border-b border-border/55 py-3 text-left last:border-0 hover:bg-muted/35"
            ...
            {quickInventory.map((item) => {
              ...
              return (
                <button key={item.id} className="grid w-full grid-cols-[48px_minmax(0,1fr)_auto] items-center gap-3 border-b border-border/55 py-2.5 text-left last:border-0 hover:bg-muted/35" onClick={() => announce(`Disponibilidade de ${item.item}`)}>
                  <div className="grid h-11 w-11 place-items-center rounded-md border border-brand/20 bg-muted text-brand">
                  ...
                  <span className={`hidden rounded-md px-2.5 py-1 text-[10px] font-medium sm:inline-flex ${low ? "bg-blush text-danger" : "bg-sage text-success"}`}>
        ...
        <article className="border border-border bg-card shadow-soft overflow-hidden rounded-xl">
          ...
          <div className="divide-y divide-border/55 px-4">
            {mockAlerts.map((alert) => {
              ...
              const tone = alert.kind === "danger" ? "bg-blush text-danger" : alert.kind === "warning" ? "bg-sand text-brand" : "bg-sage text-success";
              return (
  ...
  return (
    <>
      ...
      <div className="grid gap-3">
        {list.map((event) => (
          <article key={event.id} className="border border-border bg-card shadow-soft grid gap-4 rounded-xl p-4 sm:grid-cols-[76px_minmax(0,1fr)_auto] sm:items-center">
  ...
  return (
    <>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {list.map((item) => (
          <article key={item.id} className="border border-border bg-card shadow-soft rounded-xl p-4">
            <div className="flex items-start justify-between gap-3">
            ...
            <div className="mt-4 grid grid-cols-3 gap-2"><div className="rounded-lg bg-muted px-3 py-2"><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Total</p><p className="mt-1 font-display text-2xl">{item.total}</p></div><div className="rounded-lg bg-blush px-3 py-2"><p className="text-[10px] uppercase tracking-wide text-danger">Reservado</p><p className="mt-1 font-display text-2xl text-danger">{item.reserved}</p></div><div className="rounded-lg bg-sage px-3 py-2"><p className="text-[10px] uppercase tracking-wide text-success">Disponível</p><p className="mt-1 font-display text-2xl text-success">{item.available}</p></div></div>
...
function ProposalsPage({ query, openModal, announce }: { query: string; openModal: (kind: Exclude<ModalKind, null>) => void; announce: (message: string) => void }) {
  ...
  return (
    <>
      ...
      <div className="border border-border bg-card shadow-soft overflow-hidden rounded-xl">
  ...
  return (
    <>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.1fr_.9fr]">
        <section className="border border-border bg-card shadow-soft rounded-xl p-5">
...
function MockModal({ kind, onClose, announce }: { kind: Exclude<ModalKind, null>; onClose: () => void; announce: (message: string) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] grid place-items-center bg-overlay px-4" role="dialog" aria-modal="true">
      <div className="border border-border bg-card shadow-soft w-full max-w-lg overflow-hidden rounded-xl shadow-elevated">
...
function Dashboard() {
  ...
  const themeStyle = {
    ...
    "--foreground": "#f2e7d6",
    "--card": "#1c1b18",
    "--card-foreground": "#f2e7d6",
    "--popover": "#1b1a17",
    "--popover-foreground": "#f2e7d6",
    "--primary": "#d5b979",
    "--primary-foreground": "#211c15",
    "--secondary": "#27241f",
    "--secondary-foreground": "#f2e7d6",
    "--muted": "#24221e",
    "--muted-foreground": "#b9ad9c",
    "--accent": "#302b23",
    "--accent-foreground": "#f5ead9",
    "--border": "rgba(213,185,121,.25)",
    "--input": "rgba(213,185,121,.30)",
    "--ring": "#d5b979",
    "--sidebar": "#11110f",
    "--sidebar-foreground": "#ded1bd",
    "--sidebar-accent": "#322b20",
    "--sidebar-accent-foreground": "#f5ead9",
    "--sidebar-border": "rgba(213,185,121,.48)",
    "--brand": "#d5b979",
    "--success": "#9db18a",
    "--danger": "#df8d80",
    "--sage": "#33402e",
    ...
    "--shadow-elevated": "0 22px 64px rgba(0,0,0,.55)",
    "--shadow-button": "0 8px 24px rgba(0,0,0,.34)",
    "--hero-wash": "linear-gradient(90deg,#161512 0%,#161512 48%,rgba(22,21,18,.72) 70%,rgba(22,21,18,.16) 100%)"
  } as CSSProperties;
  ...
  return (
    <div className="min-h-screen bg-background text-foreground" style={themeStyle}>
      ...
      <div className="lg:pl-[264px]">
        <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-border/70 bg-surface/95 px-4 backdrop-blur-md sm:px-6 lg:px-7">
