...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Lumè Eventos | Painel de organização" },
      { name: "description", content: "Organize eventos, estoque, propostas e clientes com leveza em um só lugar." },
      { property: "og:title", content: "Lumè Eventos | Painel de organização" },
...
function Brand() {
  return <div className="flex items-center gap-3 lg:block lg:text-center">
    <div className="mx-auto flex h-12 w-12 items-center justify-center text-brand lg:h-14 lg:w-14"><Leaf className="h-10 w-10 stroke-[1.25]" /></div>
    <div><div className="font-display text-3xl leading-none text-foreground">Lumè</div><div className="mt-1 text-[9px] font-semibold uppercase tracking-[0.24em] text-muted-foreground">Eventos que encantam</div></div>
  </div>;
}
...
function HomePage({ query, navigate, openModal, announce }: { query: string; navigate: (page: Page) => void; openModal: (kind: Exclude<ModalKind, null>) => void; announce: (message: string) => void }) {
  ...
  return <>
    <section className="relative min-h-[126px] overflow-hidden rounded-lg border border-border bg-surface px-5 py-6 shadow-soft sm:px-7">
      ...
      <div className="relative z-10 max-w-2xl"><p className="mb-1 text-xs font-semibold uppercase tracking-[0.18em] text-brand first-letter:uppercase">{formattedDate}</p><h1 className="font-display text-4xl font-medium leading-none sm:text-5xl">Bem-vinda, Mariana!</h1><p className="mt-2 text-sm text-muted-foreground sm:text-base">Tudo o que você precisa para organizar seus eventos com tranquilidade.</p></div>
